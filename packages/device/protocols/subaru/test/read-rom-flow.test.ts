...
class TranscriptDeviceFake implements DeviceConnection {
	deviceInfo: DeviceInfo = {
		id: "fake-subaru-device",
		name: "Subaru Fake Device",
...
describe("SubaruProtocol ROM Read Flow (Integration)", () => {
	it("completes a successful ROM read handshake and first blocks", async () => {
		...
		const expectedKey = Array.from(computeSubaruKey(new Uint8Array(seed)));

		// 1. Start diagnostic session (programming mode 0x85)
		fake.addStep([0x10, 0x85], [0x50, 0x85]);
		// 2. Request Seed
		...
		fake.addStep([0x23, 0x00, 0x00, 0x00, 0x80], [0x63, ...block0Data]);

		// Block 1: Address 0x000080
		const block1Data = new Array(128).fill(0xbb);
		fake.addStep([0x23, 0x00, 0x00, 0x80, 0x80], [0x63, ...block1Data]);

		fake.addErrorStep(
			[0x23, 0x00, 0x01, 0x00, 0x80],
	...
	it("yields a meaningful error when security access is denied", async () => {
		...
		await expect(protocol.readRom(fake, vi.fn(), onEvent)).rejects.toThrow(
			"Subaru security access failed: expected 0x67 0x02, got 0x7F 0x27",
		);
