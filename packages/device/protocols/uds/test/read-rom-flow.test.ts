...
import { UdsProtocol } from "../src/index.js";
...
describe("UdsProtocol ROM Read Flow (Integration)", () => {
	it("completes a successful ROM read handshake and first blocks", async () => {
		...
		// Default computeKey XORs each seed byte with 0xFF
		const expectedKey = seed.map((b) => b ^ 0xff);
	...
	it("yields a meaningful error when security access is denied", async () => {
		...
		// 0x33 is "Security Access Denied"
